...
>
	<div class="flex flex-row-reverse md:flex-row h-works md:h-worksmd max-h-[56rem] w-full justify-center items-center">
		<div class="md:pl-44 gap-4 md:gap-5 p-4 h-full grid justify-center grid-cols-9 grid-rows-12 md:grid-cols-12 md:grid-rows-9 items-center max-w-4xl w-full sm:w-full">
			<div 
			...
			id="center" class="aspect-auto h-full overflow-hidden 
			...
			style:opacity={`calc(-50% + ${sectionScroll} * 150%)`}
			id="bottom-center" class="w-full h-full overflow-hidden 
			...
			style:opacity={`calc(-50% + ${sectionScroll} * 150%)`}
			 ...
			 col-start-6 row-start-[9] col-span-4 row-span-4 md:col-start-10 md:row-start-3 md:col-span-3 md:row-span-4
			  rounded-md">
				<CupholderModel on:click={() => zoomIn = false} imgClass="{zoomIn ? 'opacity-50': 'opacity-100'} transition-all duration-300 w-full h-full p-1 object-top object-contain text-primary-900 dark:text-primary-50" />
			...
			col-start-1 row-start-[9] col-span-5 row-span-4 md:col-start-8 md:row-start-7 md:col-span-5 md:row-span-3 bg-gradient-to-br from-primary-500 to-primary-500/20 dark:to-primary-100/70 rounded-md">
				...
				alt="" class="w-full h-full object-cover object-top pt-2 px-2 md:px-6 sepia brightness-100">
			</div>
		</div>
	</div>
</section>

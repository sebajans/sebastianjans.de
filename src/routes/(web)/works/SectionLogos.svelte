      ...
      activeLocalLogoItems.update((activeItems) => {
        ...
        discardedLogoItems.update((discardedItems) => {
          ...
          const randomActiveIndex = Math.floor(
            Math.random() * newActiveItems.length,
          );
          const randomDiscardedIndex = Math.floor(
            Math.random() * newDiscardedItems.length,
          );
...
<IntersectionObserver once threshold={0.5} {element} bind:intersecting>
  ...
  >
    <div
      class="{intersecting
        ? ''
        : '-translate-x-1/2 opacity-0'} transition-all duration-500
    ...
    >
      ...
      <div
        class="{intersecting
          ? ''
          : 'scale-75 translate-x-1/4 opacity-0'} transition-all duration-500 delay-500 w-full"
      >

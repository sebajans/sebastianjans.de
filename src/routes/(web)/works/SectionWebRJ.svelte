...
<IntersectionObserver once threshold={0.5} {element} bind:intersecting>
	<section bind:this={element} id="websites" class="relative flex flex-col">
		<div
			class="mx-auto my-auto grid h-works max-h-[56rem] grid-cols-8 grid-rows-12 gap-6 p-4 sm:grid-cols-10 sm:grid-rows-12 md:h-worksmd"
		>
			<div
				id="rena-logo"
			...
			>
				...
				>
					<div class="relative flex flex-row space-x-1 p-1 md:space-x-2 md:p-2">
						{#each Array(3) as _}
							<div
								class="dark:bg-pimary-900/80 h-1.5 w-1.5 rounded-full bg-primary-50/80 md:h-2.5 md:w-2.5"
							></div>
						{/each}
					</div>
					<img
			...
			<p
				class="{intersecting
					? ''
					: 'translate-y-20 opacity-0'} delay-250 col-[1_/_span_8] row-[10_/_span_3] h-full w-full self-center text-justify transition-all duration-1000 sm:col-[6_/_span_5] sm:row-[9_/_span_4]"
			>
				<T
					keyName="works-p-rj"
